    ...
    const fetchInvoices = async () => {
      ...
      try {
        ...
        try {
          ...
          console.error('Invalid JSON from invoices API:', responseText);
                    ...
                    <tbody className="divide-y divide-gray-700">
                      {invoices.map((invoice) => (
                        <tr key={invoice.id} className="hover:bg-slate-700/30">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">INV-{invoice.id.toString().padStart(4, '0')}</td>
